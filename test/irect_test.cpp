...
TEST(irect_test, construction3)
{
  irect value;
  EXPECT_EQ(value, irect(0, 0, 0, 0));
  EXPECT_EQ(value.x(), 0);
  EXPECT_EQ(value.y(), 0);
...
TEST(irect_test, conversion)
{
  EXPECT_EQ(irect(frect(123, 567, 800, 900)), irect(123, 567, 800, 900));
}
...
TEST(irect_test, diagonal)
{
  EXPECT_FLOAT_EQ(diagonal(irect(0, 0, 40, 30)), 50.0f);
}
